...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# TODO: This is a copy of docs/colang-2/examples/utils.py, we should unify
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
async def run_chat_interface_based_on_test_script(
    ...
    if colang:
        ...
        rails_config = RailsConfig.from_path(colang_path)
...
async def compare_interaction_with_test_script(
    ...
    result = await run_chat_interface_based_on_test_script(
        test_script, wait_time_s, colang, colang_path, llm_responses=llm_responses
    )
    clean_test_script = cleanup(test_script)
    clean_result = cleanup(result)
    if clean_test_script == clean_result:
        return None
